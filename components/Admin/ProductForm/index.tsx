...
const ProductForm: React.FC<ProductFormProps> = ({ handleSubmit, action = 'Adicionar' }) => {
  ...
  const { data: systemRequirementsData, error: systemRequirementsError } = useSwr('/admin/v1/system_requirements?length=999', SystemRequirementsService.index)
  ...
  const handleFormSubmit = (evt: React.FormEvent): void => {
    ...
    // no form data
    categories.forEach(category => formData.append(`product[category_ids][]`, category))
